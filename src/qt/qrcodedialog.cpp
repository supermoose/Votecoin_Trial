...
{
    QString ret = QString("votecoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non VTC unit input in lnReqAmount, we generate the URI with VTC as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::VTC, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
